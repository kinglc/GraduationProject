...
//        pie_chart = (PieChartView)findViewById(R.id.pie_chart);
    ...
    private float[] minToHour(int[] min){
        ...
        for(int i=0;i<4;i++){
            hour[i] = (float) min[i]/60;
            maxi = Math.max(maxi,hour[i]);
        }
        return hour;
    }
...
//        data.setAxisYLeft(axisY);
        ...
        data.setAxisYLeft(new Axis().setHasLines(true).setTextColor(Color.BLACK).setMaxLabelChars(2).setName("时长/h").setAutoGenerated(true));
...
//            String content = "user_id:" + global.getUserId();
            ...
            if (con.getResponseCode() == 200) {
                ...
                Toast.makeText(ChartActivity.this, "获取失败" + con.getErrorStream().toString(), Toast.LENGTH_SHORT).show();
            }
            con.disconnect();
